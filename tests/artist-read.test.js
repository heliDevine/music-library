...
describe('read artist', () => {
  ...
  beforeEach(async () => {
    db = await getDb();
    await Promise.all([
      db.query('INSERT INTO Artist (name, genre) VALUES(?, ?)', ['Royksopp', 'elecropop']),
      db.query('INSERT INTO Artist (name, genre) VALUES(?, ?)', ['Kylie Minogue', 'pop']),
      db.query('INSERT INTO Artist (name, genre) VALUES(?, ?)', ['Gaslight Anthem', 'rock'])
    ]),
    ...
    describe('GET', () => {
      it('returns all artist records in the database', async () => {
        ...
        res.body.forEach((artistRecord) => {
          const expected = artist.find((a) => a.id === artistRecord.id);
